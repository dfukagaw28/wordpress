...
current="$(curl -fsSL 'https://api.wordpress.org/core/version-check/1.7/' | jq -r '.offers[0].current')"-ja
sha1="$(curl -fsSL "https://ja.wordpress.org/wordpress-$current.tar.gz" | sha1sum | cut -c-40)"
